    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        btn_insert.setOnClickListener {
            if (ed_book.length() < 1 || ed_price.length() < 1) Toast.makeText(
                ...
                try {
                    dbrw.execSQL(
                        "INSERT INTO myTable(book,price) VALUES(?,?)", arrayOf<Any>(
                    ...
                    Toast.makeText(
                        this, "更新失敗$e", Toast.LENGTH_LONG).show()
                }
            }
        }
        btn_update.setOnClickListener {
            if (ed_book.length() < 1 || ed_price.length() < 1) Toast.makeText(
                ...
                try {
                    ...
                    Toast.makeText(this, ("更新書名" + ed_book.text.toString() +
                                "   價格" + ed_price.text.toString()), Toast.LENGTH_LONG).show()
                    ed_book.setText("")
                    ed_price.setText("")
                } catch (e: Exception) {
                    Toast.makeText(
        ...
        btn_delete.setOnClickListener{
            if (ed_book.length() < 1) Toast.makeText(
                this, "書名請勿留空", Toast.LENGTH_SHORT).show()
            else {
                try {
                    dbrw.execSQL("DElETE FROM myTable WHERE book LIKE '" +
                                ed_book.text.toString() + "'")

                    Toast.makeText(this, "刪除書名：" + ed_book.text.toString(),
        ...
        btn_query.setOnClickListener{
            val c: Cursor = if (ed_book.length() < 1)
